...
export async function generateMetadata({
  ...
  return buildMetadata({
    ...
    keywords: [product.name, product.sku, "marine product", "boat gear", "affiliate price check"],
  });
}
  ...
  const faqItems = [
    {
      ...
      answer: "Pricing and availability can change quickly. Use partner links for live details before checkout.",
    },
    {
      question: "Does Sea Supply Hub sell directly?",
      answer: "No. Sea Supply Hub is affiliate-first and routes purchases to partner merchants.",
    },
  ];
    ...
    <article className="space-y-6">
      ...
      <StructuredData
        data={breadcrumbSchema([
          ...
          { name: "Products", url: absoluteUrl("/categories") },
      ...
      <section className="grid gap-6 lg:grid-cols-[1fr_360px]">
        ...
        <aside className="space-y-4 rounded-2xl border border-slate-200 bg-white p-6">
          <p className="text-sm font-semibold uppercase tracking-wide text-sky-700">Affiliate Purchase</p>
          <p className="text-sm text-slate-700">Purchase is completed on partner sites. Availability may vary.</p>
          {product.partnerLinks.map((partner) => (
            <div key={partner.url} className="space-y-2 rounded-lg border border-slate-200 p-3">
              <AffiliateButton href={partner.url} label={partner.label} />
              <p className="text-xs text-slate-600">
                {partner.merchant} · Last checked {partner.lastChecked}
              </p>
            </div>
          ))}
          <p className="text-xs text-slate-500">
            Sea Supply Hub may earn a commission from qualifying partner purchases.
          </p>
        </aside>
      </section>
      ...
      <section className="grid gap-6 lg:grid-cols-2">
          ...
          <table className="mt-3 w-full text-sm">
            <tbody>
              {product.specs.map((spec) => (
                <tr key={spec.label} className="border-t border-slate-200 first:border-t-0">
                  <th className="py-2 pr-3 text-left font-medium text-slate-700">{spec.label}</th>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6">
          <h2 className="text-xl font-semibold text-slate-900">Compatibility & Guidance</h2>
          <ul className="mt-3 list-disc space-y-1 pl-5 text-slate-700">
            {product.compatibility.map((item) => (
              <li key={item}>{item}</li>
            ))}
          </ul>
          <h3 className="mt-5 text-lg font-semibold text-slate-900">Pros</h3>
          <ul className="mt-2 list-disc space-y-1 pl-5 text-slate-700">
            {product.pros.map((item) => (
              <li key={item}>{item}</li>
            ))}
          </ul>
          <h3 className="mt-4 text-lg font-semibold text-slate-900">Cons</h3>
          <ul className="mt-2 list-disc space-y-1 pl-5 text-slate-700">
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        <div className="rounded-2xl border border-slate-200 bg-white p-6">
          <h2 className="text-xl font-semibold text-slate-900">Shipping & Availability Info</h2>
          <p className="mt-2 text-sm text-slate-700">
            Orders are completed on partner sites. Shipping speed, returns, and inventory depend on the selected merchant.
            Always confirm delivery options before checkout.
          </p>
        </div>

        <div className="rounded-2xl border border-slate-200 bg-white p-6">
          <h2 className="text-xl font-semibold text-slate-900">Brand Profile</h2>
          {brand ? (
            <div className="mt-2 space-y-2 text-sm text-slate-700">
              {brand.logoUrl ? (
                <Image src={brand.logoUrl} alt={`${brand.name} logo`} width={160} height={60} className="h-12 w-auto object-contain" />
              ) : null}
              <p className="font-semibold text-slate-900">{brand.name}</p>
              <p>{brand.description}</p>
              <Link href={`/brands/${brand.slug}`} className="font-semibold text-sky-700 hover:underline">
                Explore all {brand.name} products
              </Link>
            </div>
          ) : (
            <p className="mt-2 text-sm text-slate-700">Brand profile is being expanded.</p>
          )}
        </div>
      </section>

      <section className="rounded-2xl border border-slate-200 bg-white p-6">
        <h2 className="text-xl font-semibold text-slate-900">Related Buying Guides</h2>
      ...
      <section className="rounded-2xl border border-slate-200 bg-slate-900 p-6 text-white">
        <h2 className="text-xl font-semibold">Sea Supply Hub Trust Promise</h2>
        <p className="mt-2 text-sm text-slate-200">
          We prioritize practical product fit, clear specs, and transparent affiliate disclosures so you can buy with confidence.
        </p>
      </section>
      ...
      <section>
        <h2 className="mb-4 text-2xl font-semibold text-slate-900">Related Products</h2>
